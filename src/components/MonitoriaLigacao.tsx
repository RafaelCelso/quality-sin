...
const MonitoriaLigacao: React.FC = () => {
  ...
  const handleSaveFeedback = () => {
    const now = new Date();
    const formattedDate = now.toLocaleString(); // Isso irá formatar a data e hora de acordo com as configurações locais do usuário
    const feedbackWithDate = `[${formattedDate}]\n${feedbackText}`;
    
    setFormData(prev => ({
      ...prev,
      registroFeedback: prev.registroFeedback 
        ? `${prev.registroFeedback}\n\n${feedbackWithDate}`
        : feedbackWithDate
    }));
